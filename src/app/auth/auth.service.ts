...
export class AuthService {
  ...
  signup(email: string, password: string) {
    return this.http.post<AuthResponseData>('https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=' + environment.fireBaseAPIKey,
      { email: email, password: password, returnSecureToken: true }).pipe(catchError(this.handleError), tap(respData => {
  ...
  autoLogin() {
    ...
    if (loadedUser.token) {
      this.store.dispatch(login({ email: loadedUser.email, userId: loadedUser.id, token: loadedUser.token, expirationDate: new Date(userDataJSON._tokenExpirationDate) }));
  ...
  private handleAuthentication(email: string, userId: string, token: string, expiresIn: number) {
    ...
    this.store.dispatch(login({ email: email, userId: userId, token: token, expirationDate: expirationDate }))
